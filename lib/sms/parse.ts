...
export const P = {
  ...
  toSavings:    /(?:you\s+have\s+)?(?:(?:transfer(?:r)?ed|deposited|saved|moved)\s+)?ksh\s*([\d,]+\.?\d*)\s+(?:(?:transfer(?:r)?ed|deposited|saved|moved)\s+)?(?:in)?to\s+(?:your\s+)?(kcb m-?pesa|m-?shwari)(?:\s+(?:saving(?:s)?\s+)?account)?/i,
  fromSavings:  /(?:you\s+have\s+)?(?:(?:transfer(?:r)?ed|withdrawn|moved)\s+)?ksh\s*([\d,]+\.?\d*)\s+(?:(?:transfer(?:r)?ed|withdrawn|moved)\s+)?(?:out\s+)?from\s+(?:your\s+)?(kcb m-?pesa|m-?shwari)(?:\s+(?:saving(?:s)?\s+)?account)?/i,
  receipt:      /\b([A-Z0-9]{10})\b/,
  date:         /on (\d{1,2}\/\d{1,2}\/\d{2,4})/i,
  mpesaBal:     /(?:new\s+|your\s+)?m-?pesa\s+balance\s+is\s+(?:ksh\s?)?([\d,]+\.?\d*)/i,
  kcbBal:       /(?:new\s+)?kcb m-?pesa[^.]*?balance\s*(?:is|:)?\s*(?:ksh\s*)?([\d,]+\.?\d*)/i,
