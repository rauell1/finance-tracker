      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2">
            <ArrowLeftRight className="h-5 w-5 text-[#EA580C]/40" />
            <div className="h-7 w-36 bg-[#0A0D27]/10 rounded-full" />
          </div>
          <div className="h-6 w-10 bg-[#FEF9C3] rounded-full" />
        </div>
        <div className="h-9 w-28 bg-[#EA580C]/10 rounded-lg" />
      </div>

      {/* Filter Bar Skeleton */}
      <div className="bg-white rounded-2xl border border-[#DCFCE7] shadow-sm p-3 sm:p-4">
        <div className="flex flex-wrap gap-2.5">
          <div className="relative flex-1 min-w-[180px]">
            <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 h-4 w-4 text-[#33375C]/20" />
            <div className="w-full h-9 border border-[#DCFCE7] rounded-lg bg-white" />
          </div>
          {[1, 2, 3].map((i) => (
            <div key={i} className="h-9 w-32 border border-[#DCFCE7] rounded-lg bg-white" />
          ))}
          <div className="h-9 w-32 border border-[#DCFCE7] rounded-lg bg-[#DCFCE7]/30" />
          <div className="h-9 w-32 border border-[#DCFCE7] rounded-lg bg-[#DCFCE7]/30" />
          <div className="h-9 w-20 bg-[#EA580C]/15 rounded-lg" />
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-[#DCFCE7] shadow-sm overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full border-collapse">
            <thead>
              <tr className="border-b border-[#DCFCE7] bg-[#F8F8FF] text-left">
            ...
            <tbody>
              {[1, 2, 3, 4, 5, 6].map((rowIdx) => (
                <tr key={rowIdx} className="border-b border-[#DCFCE7]/50 last:border-0 hover:bg-[#F8F8FF]/20">
                  {/* Transaction info */}
