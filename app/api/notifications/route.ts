...
export const GET = withAuth(async ({ user, supabase }) => {
  ...
  for (const o of obligations ?? []) {
    ...
    notifications.push({
      ...
      message: `${formatCurrency(Number(o.amount))} · due ${o.next_due_date}`,
  ...
  for (const b of budgets ?? []) {
    ...
    if (pct >= 80) {
      ...
      notifications.push({
        ...
        message: `${formatCurrency(spent)} / ${formatCurrency(Number(b.amount))}`,
  ...
  for (const t of largeTxns ?? []) {
    notifications.push({
      ...
      message: formatCurrency(Number(t.amount)),
      severity: "info",
    });
  }
  ...
  for (const d of debts ?? []) {
    if (d.due_date && d.due_date <= today) {
      notifications.push({
        ...
        message: `Balance: ${formatCurrency(Number(d.current_balance))}`,
        severity: "critical",
      });
    }
  }
