    ...
    <div className="min-h-screen bg-white text-[#0A0D27] antialiased overflow-x-hidden relative">
      ...
      <section className="max-w-7xl mx-auto px-6 pt-20 pb-24 text-center relative z-10">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4 mb-12">
          <Link
            href="/register"
            className="w-full sm:w-auto px-8 py-4 rounded-xl font-bold bg-[#524CF2] hover:bg-[#625DF1] text-white text-sm shadow-xl shadow-[#524CF2]/20 transition-all flex items-center justify-center gap-2"
          >
        ...
        {/* Dashboard mockup */}
        <div className="relative max-w-5xl mx-auto rounded-3xl border border-[#E2E2FF] bg-white p-2 shadow-2xl shadow-[#524CF2]/5 overflow-hidden" role="img" aria-label="FinTrack dashboard preview showing account balances and cash flow chart">
          <div className="flex items-center gap-1.5 px-5 py-3 border-b border-[#E2E2FF] bg-[#F0F0FF] rounded-t-[1.35rem]" aria-hidden="true">
          ...
          <div className="bg-[#F0F0FF]/30 p-6 text-left grid grid-cols-1 md:grid-cols-5 gap-6 select-none" aria-hidden="true">
            <div className="md:col-span-5 grid grid-cols-2 lg:grid-cols-4 gap-4">
              {[
                { label: "Net Balance", val: "KES 54,352", cls: "border-[#E2E2FF] bg-white" },
                ...
                { label: "Savings Rate", val: "28%", cls: "border-[#E2E2FF] bg-white" },
              ].map((c, i) => (
            ...
            <div className="md:col-span-3 bg-white rounded-2xl border border-[#E2E2FF] shadow-sm p-5 flex flex-col justify-between h-44">
            ...
            <div className="md:col-span-2 bg-white rounded-2xl border border-[#E2E2FF] shadow-sm p-5 flex flex-col justify-between h-44">
              ...
              <div className="space-y-2.5 mt-3">
                {[
                  ...
                  <div key={i} className="flex items-center justify-between border border-[#E2E2FF] bg-[#F0F0FF]/20 p-3 rounded-xl">
                    <span className="text-sm font-semibold text-[#0A0D27]">{w.name}</span>
                    <span className="text-xs font-bold px-2 py-0.5 rounded-lg border bg-[#F0F0FF] border-[#E2E2FF] text-[#524CF2]">{w.amt}</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        <ol className="space-y-6" aria-label="Setup steps">
          {[
            ...
            <li key={s.step} className="flex gap-6 items-start">
              <div className="h-10 w-10 rounded-full bg-[#524CF2] text-white font-black text-lg flex items-center justify-center shrink-0 shadow-md shadow-[#524CF2]/20" aria-hidden="true">{s.step}</div>
              <div>
              ...
              <ul className="space-y-3 text-sm text-[#33375C]">
                {["Manual transaction entry", "Budget charts & dashboard", "Spending category breakdown", "Budget reminders"].map((f) => (
                  <li key={f} className="flex items-center gap-3">
                    <span className="text-[#524CF2] font-bold" aria-hidden="true">✓</span>
                    <span className="font-medium">{f}</span>
                  </li>
                ))}
              </ul>
            </div>
            <Link href="/register" className="mt-8 w-full border border-[#E2E2FF] hover:bg-[#F0F0FF] text-[#0A0D27] hover:text-[#524CF2] py-4 rounded-xl text-center text-sm font-bold transition-all block">Sign up free</Link>
          </div>
