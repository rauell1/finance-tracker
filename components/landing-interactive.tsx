...
export function LandingInteractive() {
  ...
  return (
    <section id="interactive-calculator" className="max-w-4xl mx-auto px-6 py-20 relative z-10">
      <div className="bg-white rounded-[2rem] border border-[#E2E2FF] p-8 sm:p-12 shadow-xl shadow-[#524CF2]/5">
        <div className="text-center mb-10">
          <span className="text-xs font-bold uppercase tracking-wider text-[#524CF2] bg-[#F0F0FF] px-3.5 py-1.5 rounded-full border border-[#E2E2FF]">Savings Estimator</span>
            ...
            {[
              ...
              <div key={label}>
                <label className="text-xs font-bold uppercase tracking-wider text-[#33375C] flex items-center gap-2 mb-2.5">
                  <Icon className="h-4 w-4 text-[#524CF2]" aria-hidden="true" />
                  {label}
                </label>
                <div className="flex gap-4">
                  <input type="range" min={min} max={max} step={step} value={val} onChange={(e) => set(Number(e.target.value))} className="flex-1 accent-[#524CF2]" aria-label={label} />
                  <input type="number" value={val} onChange={(e) => set(Number(e.target.value))} className="w-28 bg-[#F0F0FF]/30 border border-[#E2E2FF] rounded-xl px-3 py-2 text-sm font-bold text-[#0A0D27] focus:outline-none focus:border-[#524CF2] transition-colors" aria-label={`${label} value`} />
                </div>
              </div>
            ))}
          </div>
          <div className="border border-[#E2E2FF] bg-[#F0F0FF]/25 p-8 rounded-3xl flex flex-col justify-between shadow-sm">
            <div>
              <span className="text-xs font-bold uppercase tracking-wider text-[#33375C]/75">Estimated Value</span>
              <p className="text-4xl sm:text-5xl font-black tracking-tight text-[#524CF2] mt-3" aria-live="polite">
                KES {calcResult.toLocaleString()}
              </p>
            </div>
            <div className="mt-8 pt-6 border-t border-[#E2E2FF] text-xs text-[#33375C] leading-relaxed">
              Formula: <code className="font-mono text-[#524CF2] bg-[#F0F0FF] border border-[#E2E2FF] px-1.5 py-0.5 rounded">A = P(1 + r)^t</code><br />
              Profit: <span className="font-bold text-[#524CF2]">KES {(calcResult - calcAmount).toLocaleString()}</span>
            </div>
          </div>
        </div>
      </div>
